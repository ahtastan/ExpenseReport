...
description: "Use ONLY when the user explicitly requests a pull request review, pre-merge check, or security audit of a specific PR or MR by number or URL. Do NOT trigger on general diff inspection, code exploration, or ad-hoc change questions."
---
...
# List changed files
glab mr view <MR_IID> --output json | jq -r '.changes[].new_path'
...
gh pr view $PR --json files --jq '.files[].path' | cut -d/ -f1-2 | sort -u
```

3. **Shared contracts** — types, interfaces, schemas
```bash
gh pr view $PR --json files --jq '.files[].path' | grep -E "types/|interfaces/|schemas/|models/"
```
...
CHANGED_SRC=$(gh pr view $PR --json files --jq '.files[].path' | grep -vE "\.test\.|\.spec\.|__tests__")
CHANGED_TESTS=$(gh pr view $PR --json files --jq '.files[].path' | grep -E "\.test\.|\.spec\.|__tests__")
...
gh pr view $PR --json files --jq '.files[].path' | grep -E "migrations?/|alembic/|knex/"
