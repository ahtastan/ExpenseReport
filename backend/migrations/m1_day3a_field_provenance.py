...
What this migration DOES (in a single transaction):
  ...
  1. CREATE TABLE fieldprovenanceevent (14 columns) + 7 indexes
     (1 composite ix_fieldprovenanceevent_lookup +
     6 single-column indexes from SQLModel's Field(index=True)).
     entity_type has no standalone index because it is the leftmost
     column of the composite, so a per-column index would be redundant.
...
# explicitly. Verification confirms all 7 are present post-create.
# entity_type has NO standalone index — it is the leftmost column of the
# composite ix_fieldprovenanceevent_lookup, so a per-column index would
# be redundant. Field(index=True) is omitted on entity_type in models.py.
...
CREATE_INDEX_SQL: list[str] = [
    ...
    # Also serves any entity_type-only lookup because entity_type is leftmost.
...
def migrate(db_path: str, *, apply: bool) -> MigrationResult:
    ...
    try:
        ...
        if dry_run:
            ...
            result.field_event_counts = per_field_pre
            # receipts_with_events = receipts with ANY non-NULL tracked field.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "M1 Day 3a FieldProvenanceEvent table + backfill migration. "
            "Default mode is dry-run (no changes committed); pass --apply "
            "to actually commit."
        )
    )
