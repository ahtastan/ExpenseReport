...
def test_telegram_personal_care_context_business_answer_stores_reason(monkeypatch):
    _set_reply_env(monkeypatch, enabled=True, allowlist="41001")
    fake = _install_fake_client(monkeypatch)
    ...
    with Session(engine) as session:
        ...
        session.refresh(user)
        receipt = _receipt(business_reason=None, attendees=None, report_bucket=None)
        receipt.extracted_supplier = "Rossmann"
        receipt.business_or_personal = None
        receipt.uploader_user_id = user.id
        session.add(receipt)
        session.commit()
        ...
        session.add(
            ClarificationQuestion(
                receipt_document_id=receipt.id,
                user_id=user.id,
                question_key="telegram_personal_care_context",
                question_text="If this was business spending, reply with the reason. If personal, reply personal.",
            )
        )
        session.commit()
    ...
    payload = {
        "message": {
            ...
            "from": {"id": 41001, "first_name": "Op"},
            "chat": {"id": 51001},
            "text": "business, office hygiene supplies",
        }
    }
    with Session(engine) as session:
        result = telegram_service.handle_update(session, payload)
    ...
    assert result["action"] == "answered_clarification"
    assert receipt_row.business_or_personal == "Business"
    assert receipt_row.business_reason == "office hygiene supplies"
    assert receipt_row.attendees is None
    assert receipt_row.needs_clarification is False
    assert question.status == "answered"
    assert fake.messages[-1] == "Got it. I saved that clarification."
...
def test_telegram_personal_care_context_personal_answer_closes_without_reason(monkeypatch):
    _set_reply_env(monkeypatch, enabled=True, allowlist="41001")
    fake = _install_fake_client(monkeypatch)
...
def test_live_ai_second_read_drugstore_personal_care_asks_reason_only(monkeypatch):
    _set_reply_env(monkeypatch, enabled=True, allowlist="41001", live=True)
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    fake = _install_fake_client(monkeypatch)
    _patch_extraction(
    ...
    def fake_live(**kwargs):
        return telegram_receipt_reply.agent_receipt_live_provider.LiveAgentReceiptReviewResult(
            agent_payload={
                ...
                "total_amount": "423.50",
                "currency": "TRY",
                "amount_text": "423.50 TRY",
                "line_items": [
                    {"description": "facial tissue"},
                    {"description": "hygiene personal care product"},
                ...
                "receipt_category": "market",
                "confidence": 0.91,
                "raw_text_summary": "Rossmann receipt with facial tissue, hygiene product, and hair brush.",
                "business_context_needed": False,
                "business_context_category": "market",
                "business_context_reason": "drugstore and personal care items, not snacks",
            },
            raw_response_json=json.dumps({"business_context_category": "market"}),
            prompt_text="hidden prompt",
            model_name="gpt-live-test",
    ...
    assert result["action"] == "receipt_captured"
    assert result["ai_receipt_reply_sent"] is True
    assert [question.question_key for question in questions] == ["telegram_personal_care_context"]
    assert "AI context: This looks like personal care / drugstore items." in fake.messages[-2]
    assert "market/snacks" not in fake.messages[-2]
    assert fake.messages[-1] == (
...
def test_live_ai_second_read_asks_context_for_bosnak_even_if_model_calls_it_meat_retail(monkeypatch):
    _set_reply_env(monkeypatch, enabled=True, allowlist="41001", live=True)
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    fake = _install_fake_client(monkeypatch)
    _patch_extraction(
    ...
    def fake_live(**kwargs):
        return telegram_receipt_reply.agent_receipt_live_provider.LiveAgentReceiptReviewResult(
            agent_payload={
                ...
                "confidence": 0.91,
                "raw_text_summary": "Visible receipt shows BOSNAK ET, a butcher/meat retailer rather than restaurant.",
                "business_context_needed": False,
                "business_context_category": "other",
                "business_context_reason": "meat retailer, not restaurant",
            },
            raw_response_json=json.dumps({"business_context_needed": False}),
            prompt_text="hidden prompt",
            model_name="gpt-live-test",
        )
